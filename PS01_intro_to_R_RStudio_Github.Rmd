...
Once you have staged the solution file, select "Commit". A new window will popup where you can have a final review of the changes. Write a short _commit message_ summarizing what you did, and press commit. (UPDATE IMAGE BELOW)

![](figures/git-commit.png)
In the figure above, I am set up to commit my new solution Rmd file to git. There are also some changes made to the instructions html document, but by leaving the staging box unchecked I am choosing not to commit these changes to git at this time.
...
(add something that all changes must be committed before pushing them)
